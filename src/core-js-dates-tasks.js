...
function getWeekNumberByDate(date) {
  ...
  let weekNumber = Math.floor((dayOfYear + firstDayOfWeek - 1) / 7);
  if (firstDayOfWeek <= 4) {
    weekNumber += 1;
  }
...
function getNextFridayThe13th(date) {
  ...
  while (startingDate.getDate() !== 13 || startingDate.getDay() !== 5) {
    if (startingDate.getDate() === 13) {
      startingDate.setMonth(startingDate.getMonth() + 1);
    }
    startingDate.setDate(13);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    ...
    daysCounter += 1;
    if (isWorking && daysCounter === countWorkDays) {
      isWorking = false;
      daysCounter = 0;
    } else if (!isWorking && daysCounter === countOffDays) {
      isWorking = true;
      daysCounter = 0;
    }
  }
